...
pub fn inject_ca_layer_host(context_id: ContextId, window: &Window) {
    let tao_view = get_view(window).unwrap();
    let content_view = unsafe { tao_view.superview() }.unwrap(); // It should be `contentView` of `NSWindow`.

    // Create new view for Chromium layer. We want to show wry content above.
    let mtm = MainThreadMarker::new().unwrap();
    let chromium_view = NSView::new(mtm);

    chromium_view.setFrame(tao_view.frame());
    ...
    chromium_layer.setFrame(tao_view.frame());
